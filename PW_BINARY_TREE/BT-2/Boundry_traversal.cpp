...
class Node{
    ...
    bool print_left_boundry(Node* root){
        ...
        cout<<root->val<<" ";
        if(!print_left_boundry(root->left))   return 0;
        if(!print_left_boundry(root->right))    return 0;
        return 1;  
    }
    ...
    bool print_right_boundry(Node* root,vector<int> ans){
        ...
        if(!print_right_boundry(root->right,ans))    return 0;
        if(!print_right_boundry(root->left,ans))   return 0;
        return 1;
    }
...
int main(){
    ...
    vector<int> new_tree={1,2,3,4,5,INT_MIN,6,7,INT_MIN,8,INT_MIN,9,10,INT_MIN,11,INT_MIN,12,INT_MIN
